...
### An Update 400 on an optional field may be the API REQUIRING it — fix the fixture, not the provider/SDK (runtime-verify before claiming a fix)
- trigger: an in-place Update 400s "Input should be greater than 0" / "Field required" for an Optional/Computed field the user didn't set; tempted to fix it provider-side (state-guard) or SDK-side (nullable+omitempty).
- do: FIRST decide whether the API actually REQUIRES the field on update (create-optional ≠ update-optional). Test cheaply by setting a valid value in the FIXTURE and re-sweeping. iam_user `password_reuse_count` was the whole arc: (1) fork PR #101 provider state-guard — INSUFFICIENT (state is also 0 after a create that omitted it); (2) fork PR #102 SDK `*int32`+omitempty — also wrong, it just flipped the 400 to "Field required", PROVING `UpdateIAMUser` REQUIRES the field; (3) the ACTUAL fix = `password_reuse_count = 2` in the fixture → update GREEN (run 27881799582). #101/#102 are inert and were merged prematurely — no provider/SDK change can synthesize a server-required value; only the config can. Meta-lesson: a fix isn't "done" until a sweep runtime-verifies the target stage went green — build-passing ≠ fixed; do NOT merge a provider PR before that sweep.
- conf: high · seen: 2026-06-20 · obs: 2
...
### Inline comments BETWEEN aligned HCL arguments break terraform fmt — re-run fmt after adding them
- trigger: adding an explanatory `# ...` comment between two `arg = value` lines inside a resource/block (e.g. to document a #issue); the file LOOKS fine but Static Checks goes red on `terraform fmt -check` (exit 3).
- do: terraform fmt aligns `=` only within a CONTIGUOUS run of argument lines — a comment line splits the run into separate groups, each re-aligned to its OWN longest key. Inserting a comment mid-block leaves the lines after it over-padded to the old block width. Re-run `terraform fmt` (or at minimum `-check`) on the touched scenario before pushing. Gotcha: `static.yml` only runs on `claude/**`, so unformatted code can ride a PR into `main` UNNOTICED (it did via PR #37 → red-gated every subsequent push until PR #39 fixed `scenarios/vpc_vpc_endpoint/main.tf`). terraform isn't preinstalled in the agent env — `curl` the v1.6.6 linux_amd64 zip from releases.hashicorp.com to verify locally.
- conf: high · seen: 2026-06-20 · obs: 1

### build_coverage.py merges a LIST of records, stamping last_seen=now() (most-recent-wins)
- trigger: merging a hand-built/filtered capability-matrix into coverage.json and the counts don't change (e.g. import stays flat) even though the matrix has the right stage values.
- do: the matrix file build_coverage.py consumes is a JSON **list** of `{scenario, resource, stages, note}` records — NOT a dict keyed by scenario. It stamps each record `last_seen = now()` (run_timestamp falls back to now) and only overwrites coverage.json when `now > prev.last_seen`. If you pass a **dict**, it silently merges nothing. To selectively merge (e.g. drop transient apply-fails so they don't regress lifecycle-green), filter the raw shard **lists** and re-emit a list: `[r for r in records if r['scenario'] not in drop]`. Raw downloaded shard files are already lists — pass them directly.
- conf: high · seen: 2026-06-20 · obs: 1

### Import axis: ImportStatePassthroughID greens single-id Reads; the matrix imports the FIRST state resource
- trigger: adding `ImportState` to provider resources for the #81 import axis, or reasoning about why a scenario's import=ok/fail/skip.
- do: for "bucket A" resources (Read repopulates from `state.Id` alone) the whole fix is `resource.ImportStatePassthroughID(ctx, path.Root("id"), req, resp)` + declare `resource.ResourceWithImportState` + import `framework/path` (tpf ≥1.17). The matrix import stage runs `terraform import <addr> <id>` on the scenario's **first state resource** (`primaryResourceAddrID`) — so self-contained scenarios import a *prereq* (iam_role's scenario imports its iam_policy; that's why iam_role shows iam_policy's result). import=ok needs only the import command to succeed (no re-plan). Composite-id resources (Read needs a parent id) need custom ImportState parsing, not passthrough. A **#62-class raw-type Read bug** (a field that can't hold the null/unknown the show-by-id path returns) makes import=fail with a Value Conversion Error even though apply is green — that's a pre-existing Read bug import merely exposes, fix = null-safe `types.*`.
- conf: high · seen: 2026-06-20 · obs: 1
