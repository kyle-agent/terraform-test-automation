...
| platform-unprovisionable (license / no-capacity, excluded) | 5 | - |
| **testable surface** | 82 | 100% |
| with a capability-matrix scenario run | 78 | 95% |
| reaching **validate** (green) | 78 | 95% |
| reaching **plan** (green) | 77 | 94% |
| reaching **apply** (green) | 52 | 63% |
| reaching **replan** (green) | 47 | 57% |
| reaching **destroy** (green) | 51 | 62% |
| **fully green** (all stages ok) | 46 | 56.1% |
...
Resources the dedicated test account **cannot create** for reasons outside the provider/fixtures — a vendor **license** we do not hold, or physical hardware / entitlement / **capacity** the account lacks. These are **not defects** and are excluded from the testable surface above (the funnel denominator), the same way parent-arg-only data sources are. Curated in `coverage/unprovisionable.yaml`.
