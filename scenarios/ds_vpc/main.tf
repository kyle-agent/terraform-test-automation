terraform {
  required_version = ">= 1.6"
  required_providers {
    samsungcloudplatformv2 = {
      source  = "SamsungSDSCloud/samsungcloudplatformv2"
      version = ">= 0.0.1"
    }
  }
}

provider "samsungcloudplatformv2" {}

# Read-only data-source smoke fixture (generated by scripts/gen_ds_smoke.py).
# Creates NOTHING: each block exercises a list/read endpoint with no or
# constant-only arguments, so plan/apply verify the data-source code paths.

data "samsungcloudplatformv2_vpc_internet_gateways" "smoke" {
}

data "samsungcloudplatformv2_vpc_nat_gateways" "smoke" {
}

data "samsungcloudplatformv2_vpc_ports" "smoke" {
}

data "samsungcloudplatformv2_vpc_private_nat" "smoke" {
}

data "samsungcloudplatformv2_vpc_private_nat_ips" "smoke" {
}

data "samsungcloudplatformv2_vpc_private_nats" "smoke" {
}

data "samsungcloudplatformv2_vpc_publicips" "smoke" {
}

data "samsungcloudplatformv2_vpc_subnets" "smoke" {
}

data "samsungcloudplatformv2_vpc_transit_gateways" "smoke" {
}

data "samsungcloudplatformv2_vpc_vpc_endpoints" "smoke" {
}

data "samsungcloudplatformv2_vpc_vpc_peerings" "smoke" {
}

data "samsungcloudplatformv2_vpc_vpcs" "smoke" {
}
