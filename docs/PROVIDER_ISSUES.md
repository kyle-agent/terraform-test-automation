...
| #74↝ | iam_user (Update) | in-place Update 400s **"Input should be greater than 0"** when `password_reuse_count` is unmanaged: SDK `IAMUserUpdateRequest` serializes it as a **plain `int32` (no `omitempty`)**, so `UpdateUser` always sends `0`. | open | **PR #101 (provider state-guard) is INSUFFICIENT** — state is also 0 after create. Real fix = make the SDK field nullable+omitempty (like `Description`) in iam/1.1. Surfaced by update-axis run 27875505723 |
