...
| #62↝ | iam_policy (Read/import) | `terraform import` fails with a **Value Conversion Error**: `policy_version` maps to a raw `iam.PolicyVersionResource` that **can't hold null**, and the **show-by-id** (import/refresh) response returns a **null `policy_version`** → "Received null value, however the target type cannot handle null values." Same #62 raw-type class, but on the **Read** path and **exposed by import**. (apply/replan/update are ok because create populates the field.) | open (#62-class) | exposed by the #81 ImportState work, run 27883944508. Blocks import for `iam_policy` (and `iam_role`, whose scenario imports its policy prereq). Fix = make `policy_version` null-safe (`types.*`/pointer/custom null-handling type). |
