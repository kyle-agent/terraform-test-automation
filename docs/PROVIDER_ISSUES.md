...
| #62↝ | **import-blocking #62 instances (8 resources)** | `terraform import` does a **show-by-id Read**, and several resources map a field to a **raw type that can't hold the null/unknown** that path returns → `Value Conversion Error`. Confirmed instances + null field: `iam_policy` (`policy_version`) / `iam_role` (imports its policy prereq); `dns_hosted_zone`; `virtualserver_server`; `firewall_firewall_rule` (`source_address`/`destination_address` `[]string`, already in #62); **loadbalancer**; **DBaaS** `cachestore`/`mariadb`/`postgresql` (`maintenance_option`) + `epas`/`eventstreams`/`mysql` (`init_config_option`). apply/replan/destroy are all green — these fail **only on import**. | open (#62-class) | exposed by the #81 ImportState work (runs 27883944508 / 27896616960 / 27898159015). `ImportState` is implemented (#103) so each flips to `import=ok` once its field is made null-safe (`types.*`). Consolidated on fork **#62** (comment). |
