...
#### CI에서 레지스트리 우회 (provider mirror)

`scripts/setup_provider_mirror.sh` 는 provider 플러그인을 **GitHub releases**(레지스트리가 막혀도
접근 가능)에서 받아 Terraform *filesystem mirror* 를 구성하고 `TF_CLI_CONFIG_FILE` 를
`$GITHUB_ENV` 로 export 한다. `nightly.yml` 과 `dynamic-regression.yml` 의 terraform 단계 직전에
이 스텝이 실행되므로, **러너에서 레지스트리가 403이어도 `terraform init` 이 성공**한다(레지스트리
접근이 가능하면 그대로 direct 설치로 동작 — best-effort, 다운로드 실패 시 기본 동작 유지).
버전은 `SCP_PROVIDER_VERSION` 으로 핀하거나 미지정 시 최신 릴리스를 자동 사용.

### 정기 실행 (손 안 대도 도는 회귀)

- **Nightly Integration** (`nightly.yml`): 6시간마다(`0 */6 * * *`) `scp-integration` 환경 시크릿으로
  실자원 integration 회귀. 회귀 발견 시 `publish_report.sh` 가 provider 이슈를 자동 재오픈.
- **Dynamic Regression** (`dynamic-regression.yml`): 6시간마다(3h offset) dry-run 동적 스윕.
- 둘 다 `workflow_dispatch` 로 수동 실행도 가능(즉시 한 번 돌릴 때만 사용; 정기 실행은 버튼 불필요).
